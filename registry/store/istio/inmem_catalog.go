...
func (imc *inMemoryCatalog) Register(si *ServiceInstance) (*ServiceInstance, error) {
	serviceObject := si.Service
	if serviceObject == nil {
		return nil, NewError(ErrorServiceObjectEmpty, "Service is empty", "")
	}

	if len(serviceObject.Hostname) > ServiceHostnameMaxLength {
		return nil, NewError(ErrorInstanceServiceHostnameTooLong, "Service Hostname length too long", "")
	}

	if len(serviceObject.Address) > ServiceAddressMaxLength {
		return nil, NewError(ErrorInstanceServiceAddressTooLong, "Service Address length too long", "")
	}

	if serviceObject.Ports == nil {
		return nil, NewError(ErrorInstanceServiceEmptyPorts, "Service Ports is empty", "")
	}

	if si.Endpoint == nil {
		return nil, NewError(ErrorInstanceEndpointEmpty, "Endpoint is not specified", "")
	}

	if len(si.Endpoint.Address) > EndpointAddressMaxLength {
		return nil, NewError(ErrorInstanceEndpointAddressTooLong, "Endpoint Address length too long", "")
	}

	if si.Endpoint.ServicePort == nil {
		return nil, NewError(ErrorInstanceEndpointServicePortEmpty, "Endpoint ServicePort is empty", "")
	}
